...
Fetch one or more WNBA raw games from the current static liveData CDN:
...
Expected free-throw value uses the local WNBA player-season stats file:

- `/Users/russellthomas/Docs/wnba/wnba-stats.csv`

The runner joins `FT_PERC` by `year`, `is_playoffs`, and shooter `nba_id`, converts
the percent value to a decimal, and stores it as `FTPerc` before calculating
`ExpFT`. Missing player-season FT% values still fall back to `0.75`.
...
- WNBA FT values are player-season adjusted when `wnba-stats.csv` has `FT_PERC` coverage for the shooter/year/playoff split; rows without a matched FT% still use the `0.75` fallback.
