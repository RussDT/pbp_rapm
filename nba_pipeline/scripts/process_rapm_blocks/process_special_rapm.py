"""
SPECIAL_RAPM processor.

Possession-based RAPM that uses:
- Actual points for rim attempts (2 for make, 0 for miss)
- Initial EV for non-rim FGAs (midrange and 3pt)
...
def process_special_rapm_py(file_path, year, season_str):
    """
    Processes data for Special RAPM calculation.

    Uses:
    - Actual points for rim attempts (layups, dunks, tips)
    - Initial EV for non-rim FGAs (midrange and 3pt shots)
    - Expected FT value for free throws
    ...
    if season_end_year < 2024:
        print(
            f"  Skipping SPECIAL_RAPM - only available for years 24-26 "
            f"(season end year {season_end_year} < 2024)"
        )
        return None

    print(f"  Starting SPECIAL_RAPM Processing for {season_str}...")
    ...
    print("      Calculated EOP, TeamOnOffense for SPECIAL_RAPM")
    ...
    is_make = nba_df_checked['event_type'] == 'MAKE'

    # Identify rim attempts
    if 'event_action_type' in nba_df_checked.columns:
        nba_df_checked['event_action_type_num'] = pd.to_numeric(nba_df_checked['event_action_type'], errors='coerce')
        action_type_rim = nba_df_checked['event_action_type_num'].isin(RIM_ACTION_TYPES)
    else:
        action_type_rim = pd.Series([False] * len(nba_df_checked), index=nba_df_checked.index)

    desc_combined = (nba_df_checked['home_description'].fillna('').str.lower() +
                     nba_df_checked['visitor_description'].fillna('').str.lower())
    ...
    # Calculate Special Net:
    # - FTs: use expected FT value
    # - Rim makes: 2 points
    # - Rim misses: 0 points
    # - Non-rim FGAs: use initial_ev value
    nba_df_checked['SpecialNet'] = case_when(
        is_ft_event, nba_df_checked['ExpFT'],  # FTs use expected value
        is_rim & is_make, 2.0,  # Rim makes = 2
        is_rim & ~is_make & is_fga, 0.0,  # Rim misses = 0
        is_fga, nba_df_checked['initial_ev_filled'],  # Non-rim FGAs use initial_ev
        0.0  # Everything else (turnovers, etc.)
    )

    print(f"      Calculated SpecialNet (FTs=ExpFT, Rim=actual, Non-rim=initial_ev)")

    # Calculate cumulative totals
    if group_cols:
        nba_df_checked['SpecialTotal'] = nba_df_checked.groupby(group_cols)['SpecialNet'].cumsum()
    else:
        nba_df_checked['SpecialTotal'] = nba_df_checked['SpecialNet'].cumsum()
    print("      Calculated cumulative SpecialTotal")
    ...
    print(f"      Filtered down to {len(nba_filt)} SPECIAL_RAPM End of Possession rows")

    if not nba_filt.empty:
        if 'SpecialTotal' in nba_filt.columns:
            if group_cols:
                nba_filt['Off_Diff'] = nba_filt['SpecialTotal'] - nba_filt.groupby(group_cols)['SpecialTotal'].shift(fill_value=0)
            else:
                nba_filt['Off_Diff'] = nba_filt['SpecialTotal'] - nba_filt['SpecialTotal'].shift(fill_value=0)
            nba_filt['Def_Diff'] = nba_filt['Off_Diff']
            nba_filt['Net_Diff'] = nba_filt['Off_Diff']
        print("      Calculated diffs for SPECIAL_RAPM")
    ...
    if not diff_cols_present:
        print("      Error: Could not find diff columns for SPECIAL_RAPM output.")
        nba_special_rapm_output = None
    else:
        nba_special_rapm_output = _finalize_df(nba_filt, diff_cols_present, year)

    end_time = time.time()
    print(f"  Finished SPECIAL_RAPM Processing. Time: {end_time - start_time:.2f} seconds.")
    return nba_special_rapm_output
