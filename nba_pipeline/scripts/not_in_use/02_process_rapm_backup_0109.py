...
"""
NBA RAPM Data Processing

Usage (from root):
    python nba_pipeline/scripts/02_process_rapm.py nba_pipeline/raw_data/NBA26.csv
    python nba_pipeline/scripts/02_process_rapm.py nba_pipeline/raw_data/NBA26_PS.csv
    # With luck adjustment (e.g., 50% offensive, 100% defensive):
    python nba_pipeline/scripts/02_process_rapm.py nba_pipeline/raw_data/NBA26.csv --o-luck 0.5 --d-luck 1.0
"""
...
# Action types that indicate rim attempts (layups, dunks, tips)
RIM_ACTION_TYPES = {
    # Layups
    5,    # Layup (generic)
    6,    # Driving Layup
...
def is_rim_attempt_check(event_action_type, home_desc, visitor_desc):
    ...
    # Fallback to description pattern matching
...
def process_rapm_py(file_path, year, season_str, o_luck=1.0, d_luck=1.0):
    ...
    # FTs ALWAYS use expected value (100% luck adjusted)
    # 2PT and 3PT use actual scores
    nba_df_checked['ActualNet'] = case_when(
        is_ft_event, exp_ft_value,  # FTs: Always use expected value
        is_2pt_make_event, 2.0,
        is_3pt_event, actual_score,  # 3PT: Use actual (0 or 3)
        0.0 # Default
    )

    # --- LUCK ADJUSTMENT VERSION (FT + 3PT luck adjusted) ---
    # Uses o_luck and d_luck parameters to blend expected vs actual for 3PT
    # FTs always use 100% luck adjustment regardless of o_luck/d_luck

    # Offensive luck adjustment
    nba_df_checked['LA_OffNet'] = case_when(
        is_ft_event, exp_ft_value,  # FTs: Always 100% luck adjusted
    ...
    print(f"      Calculated ActualNet (FT-adjusted) and LA_OffNet/LA_DefNet (o_luck={o_luck}, d_luck={d_luck})")
    ...
    if not nba_filt.empty:
        # --- BASE VERSION DIFFS (FT luck adjusted, no 3PT adjustment) ---
        # Off_Diff, Def_Diff, and Net_Diff all come from ActualTotal (which has FT luck adjustment)
        if 'ActualTotal' in nba_filt.columns:
            ...
            nba_filt['Def_Diff'] = nba_filt['Off_Diff']  # Same as Off_Diff for base version
            nba_filt['Net_Diff'] = nba_filt['Off_Diff']  # Net_Diff also uses FT-adjusted values
...
def process_ts_py(file_path, year, season_str):
    ...
    # --- Join Player Season Stats (FTPerc) from Supabase for FT Luck Adjustment ---
    is_playoffs = "_PS" in os.path.basename(file_path).upper()
    player_stats_df = fetch_player_stats_supabase(year, is_playoffs)
    ...
    # Identify FT events and calculate luck-adjusted action scores
    is_ft_event = series_contains(nba_df_checked['home_description'], "Free Throw", case=False) | \
                  series_contains(nba_df_checked['visitor_description'], "Free Throw", case=False) | \
                  (nba_df_checked['event_type'] == "FreeThrow")

    # For FTs: use expected value instead of actual (1 or 0)
    # For non-FTs: use actual score
    actual_action_score = (nba_df_checked['Home_Action_Score'] + nba_df_checked['Away_Action_Score']).astype(float)
    nba_df_checked['LA_Action_Score'] = np.where(is_ft_event, nba_df_checked['ExpFT'], actual_action_score)

    # Calculate luck-adjusted cumulative total
    if group_cols:
    ...
    if not nba_filt.empty:
        # Use luck-adjusted Net_Total for TS (FTs always use expected value)
...
# --- Process RIM_FREQ (Rim Frequency) Function ---
def process_rim_freq_py(file_path, year, season_str):
    ...
    # Combine both checks (either action type OR description pattern)
    ...
    d_mapping = {}
    for i in range(1, 6):
        o_mapping[f'O{i}'] = np.where(nba_df['TeamOnOffense'] == "Away", nba_df[f'a{i}'],
                                        np.where(nba_df['TeamOnOffense'] == "Home", nba_df[f'h{i}'], np.nan))
        d_mapping[f'D{i}'] = np.where(nba_df['TeamOnOffense'] == "Away", nba_df[f'h{i}'],
...
# --- Process RIM_FG_PCT (Rim FG Percentage) Function ---
...
Examples:
  ...
  python 02_process_rapm.py NBA26.csv --o-luck 0.0 --d-luck 0.0   # No LA (pure actuals)
    ...
    # Save other outputs (TS, REB, TOV, RIM_FREQ, RIM_FG_PCT)
    other_outputs = {
        "TS": ts_df,
        "REB": reb_df,
        "TOV": tov_df,
