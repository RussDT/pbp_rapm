...
def _base_processing(file_path):
    ...
    # Define group_cols early for use in scoring and shifts
    group_cols = ['game_id'] if 'game_id' in nba_df.columns else []

    # --- Calculate Base Scores from API Columns (More accurate than regex) ---
    print("      Calculating Action Scores from API score columns...")
    if group_cols:
        # Calculate diff within each game to get points scored on each row
        nba_df['Home_Action_Score'] = nba_df.groupby(group_cols)['home_score'].diff().fillna(0).astype(int)
        nba_df['Away_Action_Score'] = nba_df.groupby(group_cols)['away_score'].diff().fillna(0).astype(int)
    else:
        nba_df['Home_Action_Score'] = nba_df['home_score'].diff().fillna(0).astype(int)
        nba_df['Away_Action_Score'] = nba_df['away_score'].diff().fillna(0).astype(int)

    # Check for negative diffs (can happen if score resets or data is out of order)
    nba_df.loc[nba_df['Home_Action_Score'] < 0, 'Home_Action_Score'] = 0
    nba_df.loc[nba_df['Away_Action_Score'] < 0, 'Away_Action_Score'] = 0

    print(f"      Calculated Action Scores: Home Total={nba_df['Home_Action_Score'].sum()}, Away Total={nba_df['Away_Action_Score'].sum()}")
...
def process_rapm_py(file_path, year, season_str, o_luck=1.0, d_luck=1.0):
    ...
    # --- Identify Shot Categories for Luck Adjustment ---
    # We use the Action_Score to know IF points were scored, and descriptions to know CATEGORY.
    # This is more robust than relying on regex for the point values.
    
    # 1. Identify Free Throws
    is_ft_event = (nba_df_checked['event_type'] == "FreeThrow") | \
    ...
    # 2. Identify 3PT Attempts (Makes or Misses)
    ...
    # 3. Identify 2PT Makes
    # A 2PT make is any MAKE that is not a 3PT
    is_make_event = (nba_df_checked['event_type'] == "MAKE") | \
    ...
    is_2pt_make_event = is_make_event & ~is_3pt_event & ~is_ft_event
