...
- Treat `weighted_factors_decomp_*` as the active public NBA factor family. Both `weighted_factors_alt3_efg_value_*` and `weighted_factors_alt3_*` are legacy/audit surfaces. The active DECOMP build is the exact five-child shot tree plus `ALT_FT`, `ALT_TOV_VALUE`, and `SECOND_CHANCE_CLEAN`; see `nba_pipeline/docs/PUBLIC_DECOMP.md`.
- Treat RAPM DECOMP scposs as the current player-facing `csvs/scposs` surface, but do not feed the old Alt3 schema into it without an explicit mapper audit. Current 1Y-5Y values should come from `weighted_factors_decomp_*`; keep `TimedecayRAPM` and `*_timedecay` on their original source. `PureRAPM.csv` remains legacy compatibility. The app reads `RussDT/csvs@test-branch`, so daily `csvs` publishes should require and push `test-branch`.
- Legacy decade/support peak rows may continue to use the existing Alt3 EFG-value decade files until native DECOMP decade windows are built. Label that fallback explicitly; do not describe those files as the active public decomposition.
