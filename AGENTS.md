...
- Published `weighted_factors_alt3_*` files center the public factor columns in place by possession-weighted player means (`off_poss` for offense, `def_poss` for defense) before rebuilding `oFC` / `dFC` and residual `oSC` / `dSC`; use `nba_pipeline/scripts/center_published_alt3_weighted_factors.py` for older centered repairs, and `nba_pipeline/scripts/patch_alt3_tov_value_splits.py` when repairing rolling a2000/4000 files with the point-valued bad-pass/scoring TOV split.
